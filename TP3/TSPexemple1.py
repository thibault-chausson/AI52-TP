...
def recuit(nombre_de_ville, max_distance, T, facteur, tour):
    # recuit simulé
    ...
    cout_min_sol = cout0
    for i in range(tour):
        print('la ', i, 'ème solution = ', solution, ' donne la distance totale= ', cout0, ' la température actuelle =',
              T)
        T = T * facteur
        for j in range(50):
    ...
    print('voici la solution retenue ', min_sol, ' et son coût ', cal_distance(min_sol, distances, nombre_de_ville))


recuit(NOMBRE_DE_VILLES, MAX_DISTANCE, TEMPERATURE, FACTEUR, TOUR)
